...
def rpp(edgelist_filename, start_node=None, edge_weight='distance', verbose=False, graphml=False, max_distance=None):
    """
...
def cpp(edgelist_filename, start_node=None, edge_weight='distance', verbose=False, graphml=False, max_distance=None):
    ...
    if graphml:
        ...
        outfile.write('node1,node2,trail,distance\n')
        for edge in g.edges(data=True):
            outfile.write(str(edge[0]) + ',' + str(edge[1]) + ',0,' + str(edge[2]['distance'])+'\n')
        outfile.close()
    ...
    print 'filter time:', end - start, 'num odd node pairs:', len(odd_node_pairs)
    start = time.time()
